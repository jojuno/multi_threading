...
void *PacketThread(void *arg) {
    ...
    for (int i = 0; i < numLinesInt; i++) {
        //get line
        printf("packet thread: processing line number %d\n", i);
        if (fgets(buf, sizeof(buf), fp) != NULL) {
            if (sscanf(buf, "%d %d %d", &interArrivalTime, &tokensNeeded, &serviceTime) == 3) {
                ...
                if (My402ListEmpty(&q1) != true) {
                    printf("list not empty, adding packet to q2\n");
                    My402ListAppend(&q1, packet);
                } else if (tokens >= tokensNeeded) {
                    //tokens -= tokensNeeded;
                    printf("adding packet to q2\n");
...
void *TokenThread(void *arg) {
    ...
    while (counter < 20) {
        ...
        pthread_mutex_lock(&mutex);
        tokens++;
        printf("token thread, token %d generated\n", tokens);
        if (My402ListEmpty(&q1) != true) {
...
void *ServerThread(void *arg) {
    ...
    while (counter < 10) {
        counter++;
        printf("server thread %d, count %d\n", arg, counter);
        pthread_mutex_lock(&mutex);
        //time_to_quit
